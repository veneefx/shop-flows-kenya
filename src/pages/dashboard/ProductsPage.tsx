...
import {
  ...
  Save, X, ChevronLeft, Loader2, Tag, Upload, Star
} from "lucide-react";
          ...
          <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }}
            ...
            <div className="flex-1 p-6 space-y-4">
              ...
              {[
                ...
                { label: "SKU", key: "sku", type: "text", placeholder: "e.g. SKU-001 (optional)" },
              ...
              {/* Variants */}
              <div>
                <label className="block text-xs font-display font-semibold text-muted-foreground mb-1.5 uppercase tracking-wide">Sizes (comma separated, optional)</label>
                <input value={form.sizes} onChange={e => setForm({ ...form, sizes: e.target.value })}
                  placeholder="e.g. S, M, L, XL, 42, 43, 44"
                  className="w-full px-4 py-3 rounded-xl bg-background border border-input text-sm font-body text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
              </div>
              <div>
                <label className="block text-xs font-display font-semibold text-muted-foreground mb-1.5 uppercase tracking-wide">Colors (comma separated, optional)</label>
                <input value={form.colors} onChange={e => setForm({ ...form, colors: e.target.value })}
                  placeholder="e.g. #000000, #ff0000, #ffffff or black, red, white"
                  className="w-full px-4 py-3 rounded-xl bg-background border border-input text-sm font-body text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
                {form.colors && (
